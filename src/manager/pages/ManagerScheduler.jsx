    ...
    <div className='w-full px-8 pb-8'>
        ...
        <div className="overflow-x-auto">
          <div className="min-w-[800px]">
            <div className="grid grid-cols-8 gap-0 border-b border-gray-200 pb-2 mb-2">
              ...
              {days.map((day, i) => (
                <div key={day} className="text-center">
                  ...
                  <p className="text-xs text-gray-400">{weekDates[i]?.getDate()}</p>
                </div>
              ))}
            </div>
            ...
            {workers.length === 0 ? (
              ...
              <div key={w._id} className="grid grid-cols-8 gap-0 py-2 border-b border-gray-50 hover:bg-gray-50/50 transition-colors animate-slideUp"
      ...
      <Modal isOpen={showCreate} onClose={() => setShowCreate(false)} title="Schedule Job" size="md">
        <form onSubmit={handleCreate} className="space-y-4">
          <div>
            <label className="block text-sm font-semibold text-gray-700 mb-1">Worker ID *</label>
            <input type="text" value={form.worker} onChange={(e) => setForm(f => ({ ...f, worker: e.target.value }))}
              className="w-full bg-gray-50 rounded-lg border border-gray-200 py-2.5 px-4 text-sm outline-none focus:ring-2 focus:ring-bluelogo/30" id="sched-worker" />
          </div>
          <div>
            <label className="block text-sm font-semibold text-gray-700 mb-1">Work Order ID *</label>
            <input type="text" value={form.workOrder} onChange={(e) => setForm(f => ({ ...f, workOrder: e.target.value }))}
              className="w-full bg-gray-50 rounded-lg border border-gray-200 py-2.5 px-4 text-sm outline-none focus:ring-2 focus:ring-bluelogo/30" id="sched-wo" />
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input type="datetime-local" value={form.startTime} onChange={(e) => setForm(f => ({ ...f, startTime: e.target.value }))}
                className="w-full bg-gray-50 rounded-lg border border-gray-200 py-2.5 px-4 text-sm outline-none focus:ring-2 focus:ring-bluelogo/30" id="sched-start" />
            </div>
            <div>
              ...
              <input type="datetime-local" value={form.endTime} onChange={(e) => setForm(f => ({ ...f, endTime: e.target.value }))}
                className="w-full bg-gray-50 rounded-lg border border-gray-200 py-2.5 px-4 text-sm outline-none focus:ring-2 focus:ring-bluelogo/30" id="sched-end" />
            </div>
          </div>

        ...
        {notifications.length === 0 ? (
          ...
          <div key={n._id} className={`flex items-start gap-4 px-5 py-4 rounded-xl border transition-all duration-300 hover:shadow-md animate-slideUp ${n.isRead ? 'bg-white border-gray-100' : 'bg-bluelogo/5 border-bluelogo/10'}`}
            ...
            <div className='flex items-center gap-2 flex-shrink-0'>
              {!n.isRead && (
                <button onClick={() => handleMarkRead(n._id)} className='text-bluelogo hover:bg-bluelogo/10 p-1.5 rounded-lg transition-colors'>
                  <Icon icon="mdi:check" width={16} />
                </button>
              )}
              <button onClick={() => handleDelete(n._id)} className='text-gray-400 hover:text-red-500 p-1.5 rounded-lg transition-colors'>
